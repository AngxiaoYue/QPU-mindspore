...
def q_conj(q):
    ...
    Return:
        q_conj: np.array(..., 4)
    """
    # print(q[...,0].shape) # [5, 300, 2]
    # print(np.stack([q[..., 0], -q[..., 1], -q[..., 2], -q[..., 3]], axis=-1).shape) # [5, 300, 2, 4]
...
def q_angle(v1, v2):
    ...
    """
    v1 = normalize(v1) # [158, 2, 3]
    v2 = normalize(v2)
    # temp = np.linalg.norm(v1, axis=-1)
    # print(temp)
    # print(temp.shape) # [158, 2]
    # print(v1[..., :1].shape) # [158, 2, 1]
    q = np.repeat(np.zeros_like(v1[..., :1]), 4, axis=-1)
    # print(q.shape)  # [158, 2, 4]
    dot = np.sum(v1 * v2, axis=-1)
    # print((v1*v2).shape) # [158, 2, 3]
    # print(dot.shape) # [158, 2]

    # Inverse vectors
    inv_ind = dot < (1e-6 - 1.)
    # print(inv_ind.shape) # [158, 2]
    # print(inv_ind)
    
    if np.sum(inv_ind) > 0:
        # Get an axis and rotate pi
        # print(v1[inv_ind].shape) # [158, 3]
        # print(inv_ind)
        c = np.cross([1, 0, 0], v1[inv_ind]) 
        # If colinear, pick another axis
        # print(c.shape) # [158, 3]
        col_ind = np.linalg.norm(c, axis=-1) < 1e-6  
        # print(col_ind.shape) # [158, ]
        # print(c[..., :1].shape) # [158, 1]
        c[col_ind] = np.cross([0, 1, 0], v1[inv_ind][col_ind])
        q[inv_ind] = np.concatenate([np.zeros_like(c[..., :1]), c], axis=-1)
        # temp = np.linalg.norm(q[inv_ind], axis=-1)
        # print(temp)
    # Normal case
    norm_ind = ~inv_ind
    if np.sum(norm_ind) > 0:
        ...
        # print(c.shape)   # [316, 3]
        # print(dot[norm_ind].shape) # [316, ]
        s = np.sqrt((1 + dot[norm_ind]) * 2)  # 2 * |cos(phi)|
        s = np.expand_dims(s, axis=-1) # [316, 1]
        q_xyz = c / s
        q_w = s / 2.
        q[norm_ind] = np.concatenate([q_w, q_xyz], axis=-1)
        # temp = np.linalg.norm(q[norm_ind], axis=-1) # ???
        # print(temp)        
