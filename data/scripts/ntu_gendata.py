...
from quaternion import q_angle
...
def collect_samples(data_path, ignored_sample_path=None, benchmark='xview', part='val', sub_samples=None):
    ...
    for filename in os.listdir(data_path):
        if filename in ignored_samples:
            continue
        action_class = int(filename[filename.find('A') + 1 : filename.find('A') + 4]) # 切片操作
        subject_id = int(filename[filename.find('P') + 1:filename.find('P') + 4])
        camera_id = int(filename[filename.find('C') + 1:filename.find('C') + 4])

        if benchmark == 'xview':
            istraining = (camera_id in training_cameras) # True or False
        elif benchmark == 'xsub':
    ...
    # print("sample_name: ", sample_name) # ['S001C001P001R001A002.skeleton', 'S001C001P001R001A003.skeleton',..]
    # print("sample_label: ", sample_label) #  [1, 2, ..]
...
def gen_xyz(data_path, out_path, part, sample_name, sample_label):
    ...
    fp = open_memmap(data_save_path, dtype='float32', mode='w+', #w+是文件存在会重写，a+是追加
...
if __name__ == '__main__':
    ...
    base_dir = args.data_path.rsplit('/', 1)[0]+'/NTU-RGB-D' #从右往左切分，最多切分1次
    # print("base_dir: ", base_dir) # /home/angxiao/qpu_code/test_raw_data/NTU-RGB-D
    sub_samples=None
    xyz_dir = os.path.join(base_dir, 'xyz') # join操作会添加'/' /home/angxiao/qpu_code/test_raw_data/NTU-RGB-D/xyz
    ...
    if args.mode == 'xyz': 
        ...
        for b in benchmark:
            ...
            # print("out_path: ", out_path) # /home/angxiao/qpu_code/test_raw_data/NTU-RGB-D/xyz/xview
